...
export const createEnquiryController = async (
  ...
  try {
    ...
    const enquiry = await createNewEnquiry(validation.data);

    console.log(`[ENQUIRY] ${Date.now() - startedAt}ms`);
...
export const getEnquiry = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error && error.message === "ENQUIRY_NOT_FOUND") {
      res.status(404).json({
        success: false,
...
export const updateEnquiry = async (req: Request, res: Response) => {
  try {
    ...
    const validation = updateEnquirySchema.safeParse(req.body ?? {});
    ...
    const enquiry = await updateExistingEnquiry(id, validation.data);
 ...
 */
export const deleteEnquiryController = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const enquiry = await deleteExistingEnquiry(id);
